...
Senzor `Push status` obsahuje v atributech také poslední chybu, omezený náhled
odesílaných proměnných a omezený náhled přeskočených proměnných. Náhledy jsou
...
The `Push status` sensor also exposes the last error, a bounded preview of
pushed variables, and a bounded preview of skipped variables as attributes.
