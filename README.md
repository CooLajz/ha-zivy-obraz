...
Senzor `Push status` obsahuje v atributech také poslední chybu a omezený náhled
odesílaných proměnných. Náhled je omezený na prvních 50 proměnných, aby
zbytečně nezvětšoval stavové atributy Home Assistantu.
...
The `Push status` sensor also exposes the last error and a bounded preview of
pushed variables as attributes. The preview is limited to the first 50 variables
to avoid oversized Home Assistant state attributes.
