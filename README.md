...
- `Request batches`
- `Push problem`
- `Next push` (pouze pokud je zapnuté automatické odesílání)
...
Stejné diagnostické zařízení obsahuje také entity pro načítání dat z Export API:
...
- `Request batches`
- `Push problem`
- `Next push` (only when scheduled push is enabled)
...
The same diagnostic device also exposes Export API synchronization entities:
